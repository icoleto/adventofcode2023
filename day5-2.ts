...
let minSeedToLocationValue = null;
...
for (let i = 0; i < pairs.length; i++) {
  const pair = pairs[i];
  console.log(`${i + 1}/${pairs.length} started`);
  for (let i = 0; i < Number(pair[1]); i++) {
    let seed = Number(pair[0]) + i;
    let value = Number(seed);
    for (let map of allMaps) {
      value = map(Number(value)) || value;
    }
    if (value < minSeedToLocationValue || !minSeedToLocationValue) {
      minSeedToLocationValue = value;
    }
  }
  console.log(`${i + 1}/${pairs.length} finished`);
}

console.log(`minSeedToLocationValue: ${minSeedToLocationValue}`);
